...
const siteConfig = {
    ...
    logo: "https://plumbers-dubai.com/images/logos/logo-gpt-aqua-plumber-dubai-1785225804.webp"
};
...
window.handleLead = function(e) {
    ...
    if (window.v360Config?.sheetUrl?.length > 10) {
        ...
        // GCLID — the Google click this lead came from.
        //
        // Without it the sheet says a lead arrived but not which ad click
        // produced it, and there is then no way to tell Google weeks later
        // that this particular lead became a paying job. That upload is what
        // moves bidding from counting form fills to chasing revenue, so the
        // one field it depends on has to travel with the lead.
        //
        // The middleware injects a hidden input; localStorage is the fallback
        // for a visitor who landed on an earlier page in the same session.
        let _gclid = '';
        try {
            const _f = e?.target?.closest ? (e.target.closest('form') || e.target) : null;
            _gclid = (_f?.querySelector?.('input[name="gclid"]')?.value)
                  || (document.querySelector('input[name="gclid"]')?.value) || '';
            if (!_gclid) {
                const _st = JSON.parse(localStorage.getItem('cap_gclid') || 'null');
                if (_st && _st.v) _gclid = _st.v;
            }
        } catch (e) {}
        data.append('GCLID', _gclid);
